...
import WorkUnitListItem from './workUnitListItem';
...
class ProcessListItem extends React.Component {
  ...
  renderWorkUnits() {
    ...
    return workUnits.map((workUnit, i) => {
      const {count, name, workDefinitionKey, processDefinitionKey} = workUnit;
      return (
        <WorkUnitListItem
          name={name}
          count={count}
          ...
          workUnit={workUnit}
          selectWorkDefinition={this.props.selectWorkDefinition}
          layout={this.layout}
          key={i}
        />
      );
