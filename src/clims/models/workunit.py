...
class WorkUnit(models.Model):
    ...
    def flexible_id(self):
        """
        Because a WorkUnit can either represent an external (only in workflow engine) or internal
        (also saved locally), when creating, the user sends in the "token". The user should
        think of this token as the id, but in the backend there is a difference, because
        there are two types of ids (the internal and external ones)
        """
