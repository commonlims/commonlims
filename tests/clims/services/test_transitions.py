...
class TestTransitionService(TestCase):
    ...
    def test__create_move_transition(self):
        ...
        transition = self.ts.create(
            self.work_batch.id,
            TransitionType.MOVE,
            transition["source_location_id"],
            transition["target_location"]
        )
    ...
    @pytest.mark.skip(reason="this test should be moved to endpoint test as this validation is now done by serializer")
